...
function fetchOfferData($userid,$cleaning,$sitting,$walking){
	
	$db=users_db_connect();
	$count=0;
		
	foreach($db->query("SELECT * FROM offers JOIN registration ON userid=userid_fgn WHERE (offername='$cleaning' or offername='$sitting' or offername='$walking')and userid!='$userid'") as $row) 
	{
